...
- **The Variable Declaration Trap (CRITICAL)**: In SuperCollider, all variable declarations (`var`) MUST be placed at the very top of a function or block `{ ... }`, strictly BEFORE any executable statements or assignments. Interleaving `var` declarations with executed code will cause a fatal syntax error. NEVER declare a `var` after you have already written an executable statement in that block.
